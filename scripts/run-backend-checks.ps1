...
docker run --rm `
  ...
  -w /repo/backend `
  -e APP_ENV=local `
  -e DATABASE_URL=postgresql+psycopg://pdfextract:pdfextract@postgres:5432/pdfextract `
  -e PDFEXTRACT_TEST_DATABASE_URL=postgresql+psycopg://pdfextract:pdfextract@postgres:5432/pdfextract `
  -e S3_BUCKET_NAME=pdfextract-local `
  -e S3_ENDPOINT_URL=http://minio:9000 `
  -e PDFEXTRACT_TEST_S3_ENDPOINT=http://minio:9000 `
  -e AWS_ACCESS_KEY_ID=minioadmin `
  -e AWS_SECRET_ACCESS_KEY=minioadmin `
  -e KAFKA_BOOTSTRAP_SERVERS=kafka:29092 `
  -e PDFEXTRACT_TEST_KAFKA_BOOTSTRAP=kafka:29092 `
