...
export default function JobDetailPage({ params }: { params: { jobId: string } }) {
  ...
  const processingPercent = useMemo(() => {
    ...
    return getTimelinePercent(job.timeline, job.status);
  }, [job]);
        ...
        <div className="results-shell">
          <header className="results-header">
            <div>
              ...
              <p>{formatTimestamp(job.submitted_at)}</p>
            </div>
        ...
        <ol className="processing-steps">
          {timeline.map((item) => (
            <li key={item.stage}>
              ...
              <strong className={`processing-state is-${item.state}`}>{item.state}</strong>
            </li>
          ))}
        </ol>
...
function getTimelinePercent(items: JobTimelineItem[], status: JobDetail["status"]): number {
  if (!items.length) {
    return 0;
  }
  if (status === "completed") {
  ...
  const currentCount = items.some((item) => item.state === "current") ? 0.5 : 0;
  const percentage = ((completedCount + currentCount) / items.length) * 100;
  return Math.max(5, Math.min(95, Math.round(percentage)));
}
